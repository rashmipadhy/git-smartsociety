...
public class RunLogActivity extends AppCompatActivity {
    ...
    public static void setANDROIDID(String ANDROIDID) {
        RunLogActivity.ANDROIDID = ANDROIDID;
    }
    ...
    com.kfxlabs.smartsociety.api.GetRunLog getRunLogAPIService;
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       // imageView = findViewById(R.id.fremark_icon);
        loadingDialog = new LoadingDialog(RunLogActivity.this);

        com.kfxlabs.smartsociety.api.GetRunLog mAPIService = Api.getRunLogAPIService(RunLogActivity.this);

        setANDROIDID(Settings.Secure.getString(getApplicationContext().getContentResolver(),
                Settings.Secure.ANDROID_ID));

        getRunLogAPIService = Api.getRunLogAPIService(RunLogActivity.this);
        ...
        if (getSupportActionBar() != null) {
            ...
            getSupportActionBar().setTitle("DG Run Log");
        }
    }
        private void getDataFromServer(String dev_id) {
            Log.d("URL", "sendPost" + dev_id);

            getRunLogAPIService.savePost(deviceId).enqueue(new Callback<RunLogDetails>() {
                @Override
                public void onResponse(Call<RunLogDetails> call, Response<RunLogDetails> response) {
                    if (response.isSuccessful()){

                        recyclerView.setHasFixedSize(true);
                        recyclerView.setLayoutManager(new LinearLayoutManager(RunLogActivity.this));
                        adapter = new Run_Adapter(RunLogActivity.this,response.body().runLogList);
                        recyclerView.setAdapter(adapter);
                    }
                }

                @Override
                public void onFailure(Call<RunLogDetails> call, Throwable t) {
